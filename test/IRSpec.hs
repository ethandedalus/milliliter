...
import IR.Emit (lowerExpr, lowerStmt)
import IR.Types (Instruction (..), UnaryOperator (..), Val (..))
import qualified Parser.Types as PT (Expr (..), Stmt (..), UnaryOperator (..))
...
spec = do
  describe "lower expression" $ do
    ...
    forM_ cases $ \(name, input, expected) ->
      it ("case: " ++ name) $ evalStateT (lowerExpr input) 0 `shouldBe` expected

  describe "lower statement" $ do
    let cases =
          [ ("return literal", PT.Return (PT.Lit $ CT.LiteralInt 42), Right [Return (Lit (CT.LiteralInt 42))]),
            ( "return unary expression",
              PT.Return (PT.Unary PT.Complement (PT.Lit (CT.LiteralInt 42))),
              Right [Unary Complement (Lit $ CT.LiteralInt 42) (Var "tmp.0"), Return (Var "tmp.0")]
            ),
            ( "return nested unary expression",
              PT.Return (PT.Unary PT.Negate (PT.Unary PT.Complement (PT.Unary PT.Negate (PT.Lit $ CT.LiteralInt 42)))),
              Right

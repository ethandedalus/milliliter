...
literalInt :: Test
literalInt = UnitTest "literal int" "42" (lex >=> parse (parseExpr 0)) $ Right $ Factor (Lit (LiteralInt 42))

unaryFactor1 :: Test
unaryFactor1 = UnitTest "unary factor expression" "~42" (lex >=> parse (parseExpr 0)) $ Right $ Factor (Unary Complement (Lit (LiteralInt 42)))

unaryFactor2 :: Test
unaryFactor2 = UnitTest "unary factor expression" "~(42)" (lex >=> parse (parseExpr 0)) $ Right $ Factor (Unary Complement (Expr (Factor (Lit (LiteralInt 42)))))

simpleAddition :: Test
simpleAddition =
  UnitTest "simple 2 term arithmetic" "21 + 21" (lex >=> parse (parseExpr 0)) $
    Right $
      Binary Add (Factor (Lit (LiteralInt 21))) (Factor (Lit (LiteralInt 21)))

arith1 :: Test
arith1 =
  UnitTest "simple 3 term arithmetic" "21 + 3 * 7" (lex >=> parse (parseExpr 0)) $
    Right $
...
arithWithGrouping1 =
  UnitTest "arithmetic with grouping (1)" "10 * 3 / (7 + 11)" (lex >=> parse (parseExpr 0)) $
    Right
      ( Binary
          Div
...
arithWithGrouping2 = UnitTest "arithmetic with grouping (2)" "10 + 5 * (2 + 5) - 3" (lex >=> parse (parseExpr 0)) $ Right result
 where
  result =
    Binary
      Sub

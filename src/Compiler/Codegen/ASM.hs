...
lowerInstruction (IR.Binary op src1 src2 dst)
  | op `elem` [IR.LeftShift, IR.RightShift] = do
      op' <- liftEither $ first ConvertBinaryOperator (tryFrom op :: Either ConversionError BinaryOperator)
      src1' <- lowerVal src1
      src2' <- lowerVal src2
      dst' <- lowerVal dst
      return [Mov src1' dst', Mov src2' (Register CX), Binary op' (Register CL) dst']
lowerInstruction (IR.Binary IR.RightShift src1 src2 dst) = do
  src1' <- lowerVal src1
  src2' <- lowerVal src2
  dst' <- lowerVal dst
  return [Mov src1' dst', Mov src2' (Register CX), Binary RightShift (Register CL) dst']
